...
def test_create_invoice(auth_token):
    ...
    invoice_number = f"{now.strftime('%Y%m%dT%H%M%S')}-INVOICE_1AM"
    ...
    for key in ("id", "number", "client", "sender"):
        assert key in invoice, (
            f"В ответе invoice отсутствует поле {key}: {json.dumps(invoice, ensure_ascii=False)}"
        )

    assert invoice["number"] == invoice_number, "Номер инвойса не совпадает"
    assert invoice["client"]["id"] == client["id"], "Клиент в ответе не совпадает"
